...
def create_images_with_conf(image_path, re, label, pred_save_path, use_cropimg, args):
    ...
    if re[3] == label:
        ...
        if not os.path.exists(os.path.join(true_data_path, 'images')):
            os.makedirs(os.path.join(true_data_path, 'images'))
        ...
        if not use_cropimg:
            if not os.path.exists(os.path.join(true_data_path, 'annotations')):
                os.makedirs(os.path.join(true_data_path, 'annotations'))
        ...
        if not os.path.exists(os.path.join(true_data_path, 'inference')):
            os.makedirs(os.path.join(true_data_path, 'inference'))
        cv2.imwrite(os.path.join(true_data_path, 'inference', fn), background)

    else:
        false_data_path = str(Path(pred_save_path) / label / 'false_data' / re[3])

        if not os.path.exists(os.path.join(false_data_path, 'images')):
            os.makedirs(os.path.join(false_data_path, 'images'))
        ...
        if not use_cropimg:
            if not os.path.exists(os.path.join(false_data_path, 'annotations')):
                os.makedirs(os.path.join(false_data_path, 'annotations'))
        ...
        if not os.path.exists(os.path.join(false_data_path, 'inference')):
            os.makedirs(os.path.join(false_data_path, 'inference'))
...
def prediction_several_models_cls_name(args, device):
    ...
    for idx, ckpt in enumerate(args.resume):
        ...
        args.pred_eval_name = os.path.join(graph_save_dir)
        args.pred_save_path = os.path.join(output_dir_path)
        ...
        model.to(device)
        # print(args.drop_path, args.layer_scale_init_value, args.head_init_scale) 0.2 1e-06 1.0
        # print(1e-06 == 1e-6)
    ...
    for result_k, result_v in result_dict.items():
        ...
        if args.pred_save:
            ...
            for cls_name, data_list in pred_dataset.items():
                ...
                cls_min = 100.0
                cls_max = 0
                for cls_number in tqdm(data_list, desc=f'{cls_name} images copying... '):
                    img_path = str(cls_number[0])
                    cls_conf = float(cls_number[2])
                    annot_path = (img_path[:-3]+'txt').replace('images', 'annotations')
                    shutil.copy(cls_number[0], Path(args.pred_save_path) / cls_name / 'images')

                    if not args.use_cropimg:
                        shutil.copy(annot_path, Path(args.pred_save_path) / cls_name / 'annotations')
                    
                    if args.pred_save_with_conf:
                        create_images_with_conf(img_path, cls_number, cls_name, args.pred_save_path, args.use_cropimg, args)
        ...
        if args.pred_eval:
            ...
            if np.sum(np.array(result_v, dtype=list)[..., 2]) < 0: 
                ...
                if args.use_type:
                    class_type_dict = {}
                    with open(args.use_type_json, 'r') as f:
                        class_type_dict = json.load(f)
                    # trash
                    y_pred = [class_type_dict['trash_class_number'] if i in class_type_dict['trash'] else i for i in y_pred]
                    y_target = [class_type_dict['trash_class_number'] if i in class_type_dict['trash'] else i for i in y_target]
                        
                    # obstacle
                    y_pred = [class_type_dict['obstacle_class_number'] if i in class_type_dict['obstacle'] else i for i in y_pred]
                    y_target = [class_type_dict['obstacle_class_number'] if i in class_type_dict['obstacle'] else i for i in y_target]
                        
                    # negative
                    y_pred = [class_type_dict['negative_class_number'] if i in class_type_dict['negative'] else i for i in y_pred]
                    y_target = [class_type_dict['negative_class_number'] if i in class_type_dict['negative'] else i for i in y_target]
